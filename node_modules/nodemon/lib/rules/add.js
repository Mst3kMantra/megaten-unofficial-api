...
function add(rules, which, rule) {
  if (!{ ignore: 1, watch: 1}[which]) {
    throw new Error('rules/index.js***REMOVED***add requires "ignore" or "watch" as the ' +
      'first argument');
  }
  ...
  // this mess of replace methods is escaping "\***REMOVED***" to allow for emacs temp files

  // first up strip comments and remove blank head or tails
  rule = (rule || '').replace(reEscComments, '^^')
             .replace(reComments, '')
             .replace(reUnescapeComments, '***REMOVED***').trim();
