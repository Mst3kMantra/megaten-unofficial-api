...
***REMOVED*** dotenv [![NPM version](https://img.shields.io/npm/v/dotenv.svg?style=flat-square)](https://www.npmjs.com/package/dotenv)

<img src="https://raw.githubusercontent.com/motdotla/dotenv/master/dotenv.svg" alt="dotenv" align="right" width="200" />

Dotenv is a zero-dependency module that loads environment variables from a `.env` file into [`process.env`](https://nodejs.org/docs/latest/api/process.html***REMOVED***process_process_env). Storing configuration in the environment separate from code is based on [The Twelve-Factor App](http://12factor.net/config) methodology.
...
* [🏗️ Usage (.env)](***REMOVED***%EF%B8%8F-usage)
* [🚀 Deploying (.env.vault) 🆕](***REMOVED***-deploying)
* [🌴 Multiple Environments 🆕](***REMOVED***-manage-multiple-environments)
...
```bash
***REMOVED*** install locally (recommended)
npm install dotenv --save
```

Or installing with yarn? `yarn add dotenv`

***REMOVED******REMOVED*** 🏗️ Usage
...
.. [or using ES6?](***REMOVED***how-do-i-use-dotenv-with-import)
...
***REMOVED******REMOVED******REMOVED*** Multiline values
...
***REMOVED******REMOVED******REMOVED*** Comments

Comments may be added to your file on their own line or inline:

```dosini
***REMOVED*** This is a comment
SECRET_KEY=YOURSECRETKEYGOESHERE ***REMOVED*** comment
SECRET_HASH="something-with-a-***REMOVED***-hash"
```

Comments begin where a `***REMOVED***` exists, so if your value contains a `***REMOVED***` please wrap it in quotes. This is a breaking change from `>= v15.0.0` and on.

***REMOVED******REMOVED******REMOVED*** Parsing
...
***REMOVED******REMOVED******REMOVED*** Preload

You can use the `--require` (`-r`) [command line option](https://nodejs.org/api/cli.html***REMOVED***-r---require-module) to preload dotenv. By doing this, you do not need to require and load dotenv in your application code.
...
***REMOVED******REMOVED******REMOVED*** Variable Expansion

You need to add the value of another variable in one of your variables? Use [dotenv-expand](https://github.com/motdotla/dotenv-expand).

***REMOVED******REMOVED******REMOVED*** Syncing
...
<a href="https://github.com/dotenv-org/dotenv-vault***REMOVED***dotenv-vault-">Learn more at dotenv-vault: Deploying</a>

***REMOVED******REMOVED*** 🌴 Manage Multiple Environments
...
*ℹ️  🔐 Vault Managed vs 💻 Locally Managed: The above example, for brevity's sake, used the 🔐 Vault Managed solution to manage your `.env.vault` file. You can instead use the 💻 Locally Managed solution. [Read more here](https://github.com/dotenv-org/dotenv-vault***REMOVED***how-do-i-use--locally-managed-dotenv-vault). Our vision is that other platforms and orchestration tools adopt the `.env.vault` standard as they did the `.env` standard. We don't expect to be the only ones providing tooling to manage and generate `.env.vault` files.*

<a href="https://github.com/dotenv-org/dotenv-vault***REMOVED***-manage-multiple-environments">Learn more at dotenv-vault: Manage Multiple Environments</a>
...
[`process.env`](https://nodejs.org/docs/latest/api/process.html***REMOVED***process_process_env),
...
***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED*** debug
...
***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED*** override
...
***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED*** processEnv
...
***REMOVED******REMOVED******REMOVED******REMOVED******REMOVED*** DOTENV_KEY
...
***REMOVED******REMOVED******REMOVED*** Populate
...
***REMOVED******REMOVED******REMOVED*** Decrypt
...
console.log(decrypted) // ***REMOVED*** development@v6\nALPHA="zeta"
```

***REMOVED******REMOVED*** ❓ FAQ

***REMOVED******REMOVED******REMOVED*** Why is the `.env` file not loading my environment variables successfully?
...
***REMOVED******REMOVED******REMOVED*** Should I commit my `.env` file?
...
***REMOVED******REMOVED******REMOVED*** Should I have multiple `.env` files?
...
***REMOVED******REMOVED******REMOVED*** What rules does the parsing engine follow?

The parsing engine currently supports the following rules:

- `BASIC=basic` becomes `{BASIC: 'basic'}`
- empty lines are skipped
- lines beginning with `***REMOVED***` are treated as comments
- `***REMOVED***` marks the beginning of a comment (unless when the value is wrapped in quotes)
...
***REMOVED******REMOVED******REMOVED*** What happens to environment variables that were already set?
...
***REMOVED******REMOVED******REMOVED*** How come my environment variables are not showing up for React?
...
***REMOVED******REMOVED******REMOVED*** Can I customize/write plugins for dotenv?
...
***REMOVED******REMOVED******REMOVED*** How do I use dotenv with `import`?

Simply..

```javascript
// index.mjs (ESM)
import 'dotenv/config' // see https://github.com/motdotla/dotenv***REMOVED***how-do-i-use-dotenv-with-import
...
2. Create a separate file that will execute `config` first as outlined in [this comment on ***REMOVED***133](https://github.com/motdotla/dotenv/issues/133***REMOVED***issuecomment-255298822)

***REMOVED******REMOVED******REMOVED*** Why am I getting the error `Module not found: Error: Can't resolve 'crypto|os|path'`?
...
***REMOVED******REMOVED******REMOVED*** What about variable expansion?

Try [dotenv-expand](https://github.com/motdotla/dotenv-expand)

***REMOVED******REMOVED******REMOVED*** What about syncing and securing .env files?

Use [dotenv-vault](https://github.com/dotenv-org/dotenv-vault)

***REMOVED******REMOVED******REMOVED*** What is a `.env.vault` file?
...
***REMOVED******REMOVED*** Contributing Guide
...
***REMOVED******REMOVED*** Who's using dotenv?
